...
export function EditEventForm({ event, onSuccess, onCancel }: EditEventFormProps) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      mode: event.mode,
      registrationMethod: event.registration_method,
      description: event.description || '',
      maxParticipants: event.max_participants?.toString() || '',
      maxEntriesPerYear: event.max_entries_per_year?.toString() || '3',
      minTeamSize: event.min_team_size?.toString() || '1',
      maxTeamSize: event.max_team_size?.toString() || '1',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `Event "${formData.name}" updated successfully!`,
      });
